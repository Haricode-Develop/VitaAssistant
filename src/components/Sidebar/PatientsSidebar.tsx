...
export default function PatientsSidebar() {
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <ul className="px-2 space-y-2 pb-4 overflow-auto panel-scroll">
                {list.map(c => (
                    <li key={c.id} className={`rounded-xl border ${c.id === activeId ? "border-blue-600/40 bg-blue-50" : "border-slate-200 bg-white"}`}>
                        ...
                        <div className="flex items-center gap-2 px-3 pb-2">
                            <button className="text-xs px-2 py-1 rounded-lg border border-slate-200 hover:bg-slate-50"
                                    onClick={() => { const name = prompt("Nuevo nombre", c.patientName); if (name) renamePatient(c.id, name); }}>
                                Renombrar
                            </button>
                            <button className="text-xs px-2 py-1 rounded-lg border border-rose-200 text-rose-600 hover:bg-rose-50"
