    ...
    return (
        <div className="space-y-3">
            {parts.map((p, i) => p.type === "code" ? (
                <pre key={i} className="overflow-auto text-[12px] leading-relaxed bg-slate-900 text-slate-100 rounded-xl p-3"><code>{p.content}</code></pre>
            ) : (
...
function split(text: string): { type: "code" | "text"; content: string }[] {
    ...
    while ((m = regex.exec(text))) {
        ...
        out.push({ type: "code", content: m[0].replace(/^```[a-zA-Z0-9]*\n/, "").replace(/```$/, "") });
